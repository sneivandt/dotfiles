...
the narrowest matching skill, then load listed companions. Avoid loading
unrelated skills to reduce conflicting guidance.

## Skill index

| Skill | Use when | Companion skills |
|---|---|---|
| [`ai-tooling-apm`](./skills/ai-tooling-apm/SKILL.md) | Changing Copilot/Codex/agent/APM config, local plugins, or APM-managed skill delivery | `toml-configuration`, `config-validation`, `cross-platform-verification` |
...
| [`cross-platform-verification`](./skills/cross-platform-verification/SKILL.md) | Running the canonical local Rust/cross-platform verification sequence and wrapper checks | `testing-patterns`, `ci-cd-patterns`, `windows-specific-patterns`, `shell-patterns` |
...
| [`error-handling-patterns`](./skills/error-handling-patterns/SKILL.md) | Idempotency, dry-run, task results, and resource/task error handling | `engine-orchestration`, `logging-patterns` |
...
| [`logging-patterns`](./skills/logging-patterns/SKILL.md) | Logger usage, task recording, buffered output, or summary behavior | `engine-orchestration`, `error-handling-patterns` |
...
| [`resource-implementation`](./skills/resource-implementation/SKILL.md) | Choosing and implementing `Resource`, `IntrinsicState`, or `ResourceStateProvider` | `engine-orchestration`, `error-handling-patterns`, `testing-patterns` |
...
| [`shell-patterns`](./skills/shell-patterns/SKILL.md) | Wrapper/bootstrap shell behavior and POSIX hook script patterns | `cross-platform-verification`, `windows-specific-patterns` |
| [`sparse-checkout-patterns`](./skills/sparse-checkout-patterns/SKILL.md) | `manifest.toml` category exclusions and sparse-checkout behavior | `profile-system`, `symlink-management` |
| [`symlink-management`](./skills/symlink-management/SKILL.md) | `conf/symlinks.toml`, symlink resource behavior, and manifest alignment | `profile-system`, `sparse-checkout-patterns` |
| [`testing-patterns`](./skills/testing-patterns/SKILL.md) | Test construction/layout/snapshots and test helper usage | `cross-platform-verification`, `ci-cd-patterns` |
| [`toml-configuration`](./skills/toml-configuration/SKILL.md) | TOML section format, category filtering, and loader patterns for `conf/` and `cli/src/config/` | `config-validation`, `profile-system` |
| [`windows-specific-patterns`](./skills/windows-specific-patterns/SKILL.md) | Windows-only behavior (registry/symlink capability/platform details) | `cross-platform-verification`, `shell-patterns` |
