...
pub(in crate::logging) struct StyleChoice {
    ansi: bool,
}

/// Text styles used by the logging UI.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::logging) enum TextStyle {
    Bold,
    Dim,
    Red,
    RedBold,
...
impl StyleChoice {
    ...
    #[must_use]
    pub(in crate::logging) const fn auto(is_terminal: bool, no_color: bool) -> Self {
        Self {
    ...
    #[cfg(test)]
    #[must_use]
    pub(in crate::logging) const fn colored() -> Self {
        Self { ansi: true }
    }
    ...
    #[cfg(test)]
    #[must_use]
    pub(in crate::logging) const fn plain() -> Self {
        Self { ansi: false }
    }
    ...
    #[cfg(test)]
    #[must_use]
    pub(in crate::logging) const fn is_ansi_enabled(self) -> bool {
        self.ansi
    }

    /// Apply a text style, or return clean plain text when styling is disabled.
    #[must_use]
    pub(in crate::logging) fn paint(self, style: TextStyle, text: &str) -> String {
        if self.ansi {
    ...
    #[must_use]
    pub(in crate::logging) fn clean(self, text: &str) -> String {
        if self.ansi {
            text.to_string()
        } else {
            strip_ansi(text)
...
#[must_use]
pub(in crate::logging) fn stdout_style() -> StyleChoice {
    StyleChoice::auto(std::io::stdout().is_terminal(), no_color_enabled())
}

#[must_use]
pub(in crate::logging) fn stderr_style() -> StyleChoice {
