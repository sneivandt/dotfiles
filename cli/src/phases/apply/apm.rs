...
impl Task for InstallApmPackages {
    ...
    fn run(&self, ctx: &Context) -> Result<TaskResult> {
        ...
        ctx.debug_fmt(|| {
            format!(
                "running `apm install -g --target copilot,vscode --update` in {} (interactive \
        ...
        match ctx.executor.run_in_with_env(
            &cwd,
            "apm",
            &["install", "-g", "--target", "copilot,vscode", "--update"],
            env,
        ) {
            Ok(result) => {
...
/// Surface `apm install --update` output to the user so they can see what was
/// installed or updated.  APM provides idempotency itself via its lockfile, so
/// we have to relay its output to give the user any visibility into the install
/// — otherwise the task is silent on success.
...
mod tests {
    ...
    #[test]
    fn run_updates_apm_dependencies_while_installing() {
        let dir = tempfile::tempdir().expect("create temp dir");
        let cfg = dir.path().join(".apm").join("config");
        std::fs::create_dir_all(&cfg).expect("create ~/.apm/config dir");
        std::fs::write(
        ...
        let config = empty_config(dir.path().to_path_buf());
        let ctx = make_context(config, Platform::new(Os::Linux, false), Arc::new(mock))
            .with_home(dir.path().to_path_buf());

        let result = InstallApmPackages.run(&ctx).expect("run should not error");
        assert!(
            matches!(result, TaskResult::Ok),
            "expected Ok after apm install --update, got {result:?}"
        );
