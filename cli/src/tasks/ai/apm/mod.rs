...
//!
//! Because `apm install -g --target copilot,copilot-app` always reads
...
//! It then invokes
//! `apm install -g --target copilot,copilot-app` whenever install state
...
const APM_TARGETS: &str = "copilot,copilot-app";
...
impl Task for UpdateApmPackages {
    ...
    fn run(&self, ctx: &Context) -> Result<TaskResult> {
        if ctx.dry_run {
            ctx.log.dry_run(
                "run apm outdated -g and apm update -g --yes to advance any stale dependencies",
            );
            ctx.log.dry_run(
                "re-assert apm-managed Copilot App workflows to autopilot + enabled in \
...
/// unavailable.
///
/// When `apm update` actually runs it redeploys the experimental
...
fn run_apm_update(ctx: &Context) -> Result<ApmUpdateOutcome> {
    ...
    ctx.debug_fmt(|| {
        format!(
            "running `apm update -g --yes --target {APM_TARGETS}` in {} (interactive credential \
             prompts disabled)",
            cwd.display()
        )
    });
    match ctx.executor.run_in_with_env(
        &cwd,
        "apm",
        &["update", "-g", "--yes", "--target", APM_TARGETS],
        APM_NONINTERACTIVE_ENV,
    ) {
        Ok(result) => {
            report_apm_output(ctx, &result.stdout, &result.stderr);
