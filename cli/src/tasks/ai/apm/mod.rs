...
//! It then invokes `apm install -g --target copilot,codex` plus `copilot-app`
//! when the Copilot App database is present, whenever install state must be
//! materialized, including ordinary up-to-date runs so local symlinked plugin
//! edits are redeployed to their real runtime locations.
                ...
                .flatten()
            {
                ...
                ctx.log.info(&format!(
                    "apm install succeeded; ignoring {encode_failures} experimental \
                     ...
                     (e.g. .agent.md agents). The copilot/codex targets deployed normally; \
...
///
/// `apm install -g --target copilot,codex,copilot-app` exits non-zero when the
...
///
/// This is only meaningful for the fixed `copilot,codex,copilot-app` target
