...
impl Task for InstallAurPackages {
    ...
    fn run(&self, ctx: &Context) -> Result<TaskResult> {
        ...
        if ctx.dry_run {
            let installed = exec::run_unchecked("pacman", &["-Q"])
                .map(|r| r.stdout)
                .unwrap_or_default();
            ...
            ctx.log.info(&format!(
                "{would_install} would change, {already} already ok"
            ));
        ...
        ctx.log
            .info(&format!("{} AUR packages installed", packages.len()));
        Ok(TaskResult::Ok)
    }
}
...
impl Task for InstallParu {
    ...
    fn run(&self, ctx: &Context) -> Result<TaskResult> {
        if exec::which("paru") {
            return Ok(TaskResult::Skipped("paru already installed".to_string()));
        }
...
fn install_pacman(
    ...
    ctx.log
        .info(&format!("{} packages installed", packages.len()));
    Ok(TaskResult::Ok)
}
...
fn install_winget(
    ...
    if ctx.dry_run {
        let list = exec::run_unchecked("winget", &["list"])
            .map(|r| r.stdout)
            .unwrap_or_default();
        let mut would_install = 0u32;
        let mut already = 0u32;
        for pkg in packages {
            if list.contains(&pkg.name) {
                ctx.log
                    .debug(&format!("ok: {} (already installed)", pkg.name));
                already += 1;
            } else {
                ctx.log.dry_run(&format!("would install: {}", pkg.name));
                would_install += 1;
            }
        }
        ctx.log.info(&format!(
    ...
    for pkg in packages {
        ...
        if result.success {
            ...
            ctx.log
                .debug(&format!("winget: {} may already be installed", pkg.name));
        }
    }

    ctx.log.info(&format!("{installed} packages installed"));
    Ok(TaskResult::Ok)
}
