...
- `apm` — Update and install AI plugin manifests via Microsoft APM (`apm install -g --target copilot,vscode --update`; reads `~/.apm/apm.yml` generated by merging every `~/.apm/config/*.yml` fragment, e.g. the repo's `symlinks/apm/config/base.yml`)
