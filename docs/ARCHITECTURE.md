...
- `apm` — Install and update AI plugin manifests via Microsoft APM (reads `~/.apm/apm.yml` generated by merging every `~/.apm/config/*.yml` fragment; runs `apm experimental enable copilot-app` before `apm install -g --target copilot,copilot-app,vscode` when the manifest or lockfile changed, otherwise checks `apm outdated -g` before `apm deps update -g --target copilot,copilot-app,vscode`)
