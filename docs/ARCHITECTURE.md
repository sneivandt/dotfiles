...
- `apm` (ai) — `InstallApmPackages`: converge AI plugin manifests via Microsoft APM (reads `~/.apm/apm.yml` generated by merging every `~/.apm/config/*.yml` fragment; runs `apm install -g --target copilot,copilot-app` when the manifest, lockfile, or local plugin content needs redeploying). Convergence only — it never advances locked refs
