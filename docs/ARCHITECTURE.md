...
- **`install.rs`** — Uses `CommandRunner` to resolve the profile, load `Config`, build the task list, filter by `--skip`/`--only`, and execute each task via `tasks::execute()`. Exposes `run_pipeline(advance_versions)`, the shared implementation behind both `install` and `update`
- **`update.rs`** — Delegates to `install::run_pipeline` with `advance_versions = true`, so it runs the identical task graph as `install` but additionally advances pinned dependency versions (currently the APM dependency refresh)
...
- `apm` (ai) — `InstallApmPackages`: converge AI plugin manifests via Microsoft APM (reads `~/.apm/apm.yml` generated by merging every `~/.apm/config/*.yml` fragment; runs `apm install -g --target copilot,vscode,copilot-app` when the manifest or lockfile changed). Convergence only — it never advances locked refs
...
- `apm` (ai) — `UpdateApmPackages`: runs `apm outdated -g` and, when stale, `apm deps update -g` to advance locked dependency refs. Self-guards on the install success marker so it only advances after a successful convergence
