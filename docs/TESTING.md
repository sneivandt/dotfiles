...
- `phases/bootstrap/*.rs`, `phases/repository/*.rs`, `phases/apply/*.rs` — Task `should_run` logic and helper functions
...
Task tests use context builder helpers defined in `phases/mod.rs` (available in `#[cfg(test)]` scope):
