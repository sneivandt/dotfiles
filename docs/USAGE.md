...
17. **Install APM Packages** - Runs `apm install -g --target copilot,vscode` to deploy AI plugin manifests defined in `symlinks/apm/apm.yml` (linked to `~/.apm/apm.yml`) globally under `~/.copilot/` and `~/.vscode/` ([Microsoft APM](https://github.com/microsoft/apm))
...
13. **Install APM Packages** - Runs `apm install -g --target copilot,vscode` to deploy AI plugin manifests defined in `symlinks/apm/apm.yml` (linked to `~/.apm/apm.yml`) globally under `~/.copilot/` and `~/.vscode/` ([Microsoft APM](https://github.com/microsoft/apm))
