...
17. **Install APM Packages** - Merges every `~/.apm/config/*.yml` fragment (e.g. `symlinks/apm/config/base.yml`, plus any overlay fragments) into `~/.apm/apm.yml`, runs `apm experimental enable copilot-app`, installs when the manifest or lockfile changed, and otherwise uses `apm outdated -g` before running `apm deps update -g --target copilot,copilot-app,vscode` only when locked dependencies are stale ([Microsoft APM](https://github.com/microsoft/apm))
