...
| User | 13 | APM Packages | Merges every `~/.apm/config/*.yml` fragment into `~/.apm/apm.yml`, runs `apm experimental enable copilot-app`, installs when the manifest or lockfile changes, and otherwise uses `apm outdated -g` before running `apm deps update -g --target copilot,copilot-app,vscode`. | Idempotent via APM's lockfile. |
...
- `Install APM packages` runs `apm experimental enable copilot-app` before APM target operations, then runs `apm install -g --target copilot,copilot-app,vscode` when the merged manifest or lockfile changed; otherwise it checks `apm outdated -g` and only runs `apm deps update -g --target copilot,copilot-app,vscode` when locked dependencies are stale
