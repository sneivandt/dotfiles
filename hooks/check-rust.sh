...
#                                              (cross-platform linting, optional)
#   4. PSScriptAnalyzer                        (PowerShell linting, on .ps1/.psm1)
#
# Cross-target clippy and PSScriptAnalyzer are skipped (with a notice) when the
# required toolchain is not installed locally, so the hook works on minimal
# machines while still catching Windows-only failures when tooling is present.
#
...
if printf '%s\n' "$STAGED" | grep -q '\.rs$'; then
  ...
  if rustc --print target-list 2>/dev/null | grep -qx "$WIN_TARGET" \
    ...
    if ! cargo clippy --manifest-path "$MANIFEST" --target "$WIN_TARGET" --all-targets -- -D warnings 2>&1; then
      printf '\n%s======================================================%s\n' "$RED" "$NC"
