...
#
# Initialize any git submodules declared in conf/submodules.ini when
# status indicates they are uninitialized or out of date (+ or - markers).
# Uses recursive init to support nested submodules.
install_git_submodules()
{(
  if [ ! -d "$DIR"/.git ]; then
  ...
  # Check if submodules.ini exists (may be excluded by sparse checkout)
  if [ ! -f "$DIR"/conf/submodules.ini ]; then
    log_verbose "Skipping git submodules: no submodules.ini found"
    return
  fi
  ...
  # are initialized regardless of active profile as they may be needed for
  # repository integrity and testing purposes.
  sections="$(grep -E '^\[.+\]$' "$DIR"/conf/submodules.ini | tr -d '[]')"

  for section in $sections
  do
    read_ini_section "$DIR"/conf/submodules.ini "$section" | while IFS='' read -r module || [ -n "$module" ]
    do
      if [ -z "$module" ]; then
        continue
      fi
      # Validate module exists in .gitmodules
      if [ -f "$DIR"/.gitmodules ]; then
        if ! git -C "$DIR" config -f .gitmodules --get-regexp "submodule\\..*\\.path" | grep -q "$module\\$"; then
          log_verbose "Warning: submodule $module in submodules.ini not found in .gitmodules" >&2
        fi
      fi
      echo "$module"
    done
...
#
# Update git submodules declared in conf/submodules.ini, pulling
...
{(
  ...
  # Check if submodules.ini exists (may be excluded by sparse checkout)
  if [ ! -f "$DIR"/conf/submodules.ini ]; then
    log_verbose "Skipping update git submodules: no submodules.ini found"
    return
  fi
  ...
  # Read submodules from all sections in submodules.ini
  # Note: Submodules are intentionally not filtered by profile - all submodules
  # are updated regardless of active profile to maintain repository consistency.
  sections="$(grep -E '^\[.+\]$' "$DIR"/conf/submodules.ini | tr -d '[]')"
