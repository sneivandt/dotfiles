# https://raw.githubusercontent.com/robbyrussell/oh-my-zsh/master/plugins/golang/golang.plugin.zsh
...
if command -v go >/dev/null 2>&1 && [ -n "$(go env GOPATH)" ]; then
  alias gop='cd $(go env GOPATH)'
  alias gopb='cd $(go env GOPATH)/bin'
  alias gops='cd $(go env GOPATH)/src'
fi